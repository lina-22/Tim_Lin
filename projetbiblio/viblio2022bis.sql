...
--
-- Client :  127.0.0.1
-- Généré le :  Jeu 10 Février 2022 à 23:38
...
-- Procédures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `emprunte`(numa int(4), cot int(4))
begin
-- ajout d'un emprunt
insert into emprunt(numadh,cotelivre,datemp) values(numa,cot,curdate());
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `emprunter`(IN `numa` INT(4), IN `cot` INT(4))
begin
-- on ajoute un nouvel emprunt en renseignant
-- le N° adherent et la cote du livre emprunté
insert into emprunt values(null,numa,cot,curdate(),null);
-- on met à jour l'état du livre emprunté
update livre
set etat=1
where cote=cot;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `restituer`(nume int(6))
begin
-- déclaration de variable
declare coteli int(4);

-- on exécute la requête pour récupérer la cote du livre à restituer
-- et la stocke dans la variable déclarée cot
select cotelivre into coteli from emprunt 
where num=nume;
...
-- on met à jour l'état du livre restitué
update livre
set etat=0
where cote=coteli;
...
-- Fonctions
--
CREATE DEFINER=`root`@`localhost` FUNCTION `adhesion`(`numa` INT(4)) RETURNS int(4)
begin
...
CREATE DEFINER=`root`@`localhost` FUNCTION `auth`(`adr` VARCHAR(30), `pw` VARCHAR(40)) RETURNS int(1)
begin
...
CREATE DEFINER=`root`@`localhost` FUNCTION `edit`(`cotelivre` INT(4)) RETURNS varchar(30) CHARSET utf8
begin
...
(139, 'JavaScript', 2021, 0, ' 139', 'simple', 'C1', 'e2');
...
--
-- Doublure de structure pour la vue `v_livredit`
--
CREATE TABLE IF NOT EXISTS `v_livredit` (
`cote` int(4)
,`titre` varchar(30)
,`annee` int(4)
,`etat` int(1)
...
--
-- Structure de la vue `v_livredit`
--
DROP TABLE IF EXISTS `v_livredit`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_livredit` AS select `livre`.`cote` AS `cote`,`livre`.`titre` AS `titre`,`livre`.`annee` AS `annee`,`livre`.`etat` AS `etat`,`livre`.`photo` AS `photo`,`livre`.`commentaire` AS `commentaire`,`livre`.`idcateg` AS `idcateg`,`livre`.`codedit` AS `codedit`,`editeur`.`nomedit` AS `nomedit` from (`livre` join `editeur` on((`livre`.`codedit` = `editeur`.`code`))) order by `livre`.`titre`;
